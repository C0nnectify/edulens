            ...
            try {
              ...
              if (stored) {
                ...
                if (!parsed.documentType || parsed.documentType === 'lor') {
                  const draft = parsed.documentDraft || (parsed as unknown as { editor_json?: Record<string, unknown>; html?: string });
        ...
        <div className="max-w-7xl mx-auto p-4 lg:p-8 space-y-6 lg:space-y-8">
          ...
          <div className="grid lg:grid-cols-2 gap-4 lg:gap-8">
            ...
            <div className="space-y-4 lg:space-y-6">
              ...
              <Card className="p-4 lg:p-6 space-y-3 lg:space-y-4 shadow-md hover:shadow-lg transition-shadow">
                <div className="flex items-center gap-2 pb-2 border-b">
                  <div className="w-7 h-7 lg:w-8 lg:h-8 rounded-full bg-blue-100 flex items-center justify-center text-blue-600 font-semibold text-sm lg:text-base">1</div>
                  <h2 className="text-lg lg:text-xl font-semibold text-gray-800">Recommender Details</h2>
                </div>
                <div className="space-y-3">
                  <div>
                    <label className="text-sm font-medium text-gray-700 mb-1.5 block">Name</label>
                    <Input placeholder="Enter recommender's full name" value={recommenderName} onChange={e=>setRecommenderName(e.target.value)} className="h-10 lg:h-11" />
                  </div>
                  <div>
                    <label className="text-sm font-medium text-gray-700 mb-1.5 block">Designation & Organization</label>
                    <Input placeholder="e.g., Professor, Computer Science Department" value={recommenderTitle} onChange={e=>setRecommenderTitle(e.target.value)} className="h-10 lg:h-11" />
                  </div>
          ...
          <div className="flex justify-center pt-4">
            <Button 
              ...
              size="lg"
              className="w-full max-w-md h-11 lg:h-12 text-base lg:text-lg font-semibold bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-700 hover:to-purple-700 disabled:opacity-50"
            >
              {isGenerating ? (
                <>
        ...
        <div className="h-[calc(100vh-4rem)] flex flex-col">
          <div className="bg-white/90 border-b border-slate-200/60 px-3 lg:px-4 py-2 lg:py-3 flex flex-wrap items-center justify-between gap-2 backdrop-blur">
            <div className="flex items-center gap-2 lg:gap-3">
              ...
              <div>
                <h2 className="text-sm lg:text-lg font-semibold">Letter of Recommendation</h2>
            ...
            <div className="flex gap-1 lg:gap-2 items-center flex-wrap">
              {existingLORs.length > 0 && (
                <select
                  className="text-xs lg:text-sm border rounded px-1 lg:px-2 py-1 max-w-[100px] lg:max-w-none"
                  value={activeLORId || ''}
              ...
              {saveMessage && <span className="text-xs lg:text-sm text-green-600">{saveMessage}</span>}
              ...
              <Button variant="destructive" size="sm" onClick={handleDelete} disabled={!lorId} className="px-2 lg:px-3">
            ...
            <div className="flex-1 overflow-hidden">
              {!mobileEditorMode ? (
                ...
                <div className="h-full overflow-auto bg-white">
                  <Editor ref={editorRef} initialContent={initialEditorContent || undefined} />
                </div>
              )}
            </div>
          </div>
